...
import glob
from avgn.signalprocessing.spectrogramming import spectrogram_librosa
from avgn.signalprocessing.spectrogramming_scipy import _db_to_amplitude, build_mel_basis, build_mel_inversion_basis, \
...
def single_file_test(sr, num_mel_bins, n_fft, mel_lower_edge_hertz, mel_upper_edge_hertz, hop_length_ms, win_length_ms,
    ...
    mel_inv = griffinlim_sp(
        _mel_to_linear(debug_info['mel'],
                       _mel_inverse_basis=mel_inversion_basis),
        hparams.sr, hparams)
    sf.write(f'{dump_folder}/{index}_mel.wav', mel_inv, samplerate=hparams.sr)
