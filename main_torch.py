...
def main(config, load, train):
    ...
    (
        ...
        model_path,
        config_path,
    ) = get_model_and_dataset(config=config, loading_epoch=load)
        ...
        for ind_epoch in range(config["num_epochs"]):
            ...
            if ind_epoch % 10 == 0:
                ...
                test_dataloader = get_dataloader(
                    dataset_type=config["dataset"],
                    dataset=dataset_val,
                    batch_size=num_examples_plot,
                    shuffle=True,
                )
                savepath = (
                    f"{model.model_dir}/training_plots/{ind_epoch}/reconstructions"
                )
    ...
    if USE_CUSTOM_SAMPLE:
        ...
        for example_ind, examples_dict in data_source.items():
            for name in ["start", "end"]:
                ...
                if name == "start":
                    start_data.append(
                        SpectroDataset.process_mSp(
                            mSp,
                            chunk_len_win=hparams["chunk_len_win"],
    ...
    if not os.path.isdir(savepath):
        os.mkdir(savepath)
    plot_reconstruction(
        model, hparams, gen_dataloader, savepath, custom_data=custom_data
    )
