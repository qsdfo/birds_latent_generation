...
config = {
    ...
    'encoder_kwargs': dict(
        ...
        conv2z=[
            nn.Linear(in_features=(h_dim // (2 ** 5)) * \
                      (w_dim // (2 ** 5)) * 256, out_features=1024),
            nn.ReLU(),
            nn.Dropout(p=0.1),
    ...
    'decoder_kwargs': dict(
        ...
        deconv_stack=[  # (b, 256, 1, 8)
            ...
            # nn.ConvTranspose2d(256, 1, (1, 1), stride=(1, 1))
            #
            nn.ConvTranspose2d(deconv_input_shape[0], 256, (2, 4), stride=(2, 4)),  # # (, , 2, 5) -> (, , 4, 20)
            nn.ConvTranspose2d(256, 256, (2, 2), stride=(1, 1)),  # (, , 5, 21)
            nn.ConvTranspose2d(256, 256, (3, 3), stride=(3, 3)),  # (, , 15, 63)
            nn.ConvTranspose2d(256, 256, (2, 2), stride=(1, 1)),  # (, , 16, 64)
            nn.ConvTranspose2d(256, 256, (2, 2), stride=(2, 2)),  # (, , 32, 128)
            nn.ConvTranspose2d(256, 256, (2, 2), stride=(2, 2)),  # (, , 64, 256)
            nn.ConvTranspose2d(256, 1, (1, 1), stride=(1, 1))
        ]
    ),
    'model_kwargs': dict(
