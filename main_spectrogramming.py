...
from avgn.signalprocessing.spectrogramming_scipy import (
    build_mel_basis,
    build_mel_inversion_basis,
    spectrogram_sp,
    inv_spectrogram_sp,
)
...
def main(
    ...
    if chunk_len["type"] == "ms":
        ...
        chunk_len_win = (
            round((chunk_len_samples_not_rounded - win_length) / hop_length) + 1
        )
    ...
    for key in syllable_df.key.unique():
        ...
        for syll_ind, (st, et) in enumerate(
            zip(this_syllable_df.start_time.values, this_syllable_df.end_time.values)
        ):
            ...
            for time_shift in time_shifts:
                for pitch_shift in pitch_shifts:
                    ...
                    if debug and (syll_ind in ind_examples):
                        ...
                        # normalised audio
                        sf.write(
                            f"{dump_folder}/{syll_ind}_{time_shift}_{pitch_shift}_sn.wav",
                            sn,
                        ...
                        plt.savefig(
                            f"{dump_folder}/{syll_ind}_{time_shift}_{pitch_shift}_mS.pdf"
                        )
                        plt.close()
                        ...
                        sf.write(
                            f"{dump_folder}/{syll_ind}_{time_shift}_{pitch_shift}_mS.wav",
                            audio_reconstruct,
                            samplerate=hparams.sr,
                        )
...
if __name__ == "__main__":
    ...
    power_l = [1.5]
    ref_level_db_l = [-20]
    chunk_len = {"type": "stft_win", "value": 256}
    ...
    for (
        ...
        num_mel_bins_l,
        n_fft_l,
        mel_lower_edge_hertz_l,
        mel_upper_edge_hertz_l,
        hop_length_l,
        win_length_l,
        ref_level_db_l,
        power_l,
